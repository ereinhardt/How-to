...
export default async function start_http_server(
  ...
  server.get("/:id/:filename", async (req, res) => {
    try {
      ...
      const file_path = p.join(__dirname, "../../", "users", id, filename);

      // m3u8 playlists must never be cached (they change constantly during streaming)
      ...
      res.sendFile(file_path, (err) => {
        if (err) {
          debug_error(`ERROR sending file ${file_path}:`, err.message);
          if (!res.headersSent) {
      ...
      debug_error(`CRITICAL ERROR in /:id/:filename route:`, error.message);
      if (!res.headersSent) {
  ...
  server.get(`/:video_id/:user_id/:filename`, (req, res) => {
    try {
      ...
      if (
        ...
        let nextVideoId = video_id;
        let nextSegmentNumber = segment + 1;
        ...
        if (next_segment == "ENDING") {
          ...
          if (!newQuestion) {
            ...
            debug_log("NEW QUESTION ID!", newQuestion);
            nextVideoId = newQuestion.id;
            nextSegmentNumber = 0;
      ...
      debug_error(`CRITICAL ERROR in video route:`, error.message);
      if (!res.headersSent) {
        res.status(500).send("Internal server error");
      }
    }
